...
#else
extern const typeof(timer_of_match) __mod_of__timer_of_match_device_table		\
  __attribute__ ((unused, alias(__stringify(timer_of_match))));
...
void mxc_dump_gpt_reg(void)
{
	int i;
	uint32_t tstat;
	struct imx_timer *imxtm ;
	struct icap_channel* ic = &icap_channel[0];
	imxtm = ic->imxtm;
  pr_info("GPT Regs:");
	for(i=0;i<10;i++)
	{
 			if( (i%4) ==0) pr_info("\n" );
 		tstat = readl_relaxed(imxtm->base + i*4);
 		pr_info("%08x ",tstat);
 	
	}
}
EXPORT_SYMBOL_GPL(mxc_dump_gpt_reg);